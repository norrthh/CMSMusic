...
const {events, music_id_last} = defineProps({
    events: {type: Object},
...
<template>
    ...
    <div class="mb-4">
        <h1 class="title mb-2">Сменалар һәм аланнар</h1>

        <div class="flex gap-4">
            <Link v-for="event in events" :href="`/event/${event.id}`" >
                <img class="w-64 h-64 rounded-xl" :src="event.photo" alt="">
                <p class="text-center mt-2 mb-2 text-gray-400 font-semibold">{{ event.name }}</p>
            </Link>
        </div>
    </div>
